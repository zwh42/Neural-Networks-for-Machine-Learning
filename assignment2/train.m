...
learning_rate = 0.1;%0.1; %0.1;  % Learning rate; default = 0.1.
momentum = 0.9; %0.9;  %0.9;  % Momentum; default = 0.9.
numhid1 = 50; %50;  % Dimensionality of embedding space; default = 50.
numhid2 = 200; %200;  % Number of units in hidden layer; default = 200.
init_wt = 0.01;  % Standard deviation of the normal distribution
                 % which is sampled to get the initial weights; default = 0.01
disp("momentum:"), disp(momentum);
...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    %disp("embed_to_hid_weights_gradient dim: "), disp(size(embed_to_hid_weights_gradient));
    %disp("back_propagated_deriv_1 dim: "), disp(size(back_propagated_deriv_1));
    %disp("embedding_layer_state dim: "), disp(size(embedding_layer_state));
    ...
    %disp("\nhid_bias_gradient dim = "), disp(size(hid_bias_gradient));
    %disp("sum(back_propagated_deriv_1, 2) dim = "), disp(size(sum(back_propagated_deriv_1, 2)));
    %disp("sum(back_propagated_deriv_1, 1) dim = "), disp(size(sum(back_propagated_deriv_1, 1)));
    % Options
    ...
    %disp("\nback_propagated_deriv_2 dim = "), disp(size(back_propagated_deriv_2));
    %disp("a dim = "), disp(size(embed_to_hid_weights * back_propagated_deriv_1));
    %disp("b dim = "), disp(size(back_propagated_deriv_1 * embed_to_hid_weights));
    %disp("c dim = "), disp(size(back_propagated_deriv_1' * embed_to_hid_weights));
    %disp("d dim = "), disp(size(back_propagated_deriv_1 * embed_to_hid_weights'));
