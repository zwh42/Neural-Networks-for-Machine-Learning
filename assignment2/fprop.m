...
%disp( "\nhidden_layer_state:" ), disp(size(hidden_layer_state));
...
%disp("\ninputs_to_softmax size: "), disp(size(inputs_to_softmax));
%disp(size(hid_to_output_weights)), disp(size(hidden_layer_state)), disp(size(repmat(output_bias, 1, batchsize)));
